...
{
    if (!_userNameLabel) {
        _userNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 20, kScreenWidth, 20)];
        _userNameLabel.backgroundColor = [UIColor clearColor];
        _userNameLabel.text = @"用户名：13162521916";
        _userNameLabel.textAlignment = NSTextAlignmentCenter;
        _userNameLabel.font = font14;
        _userNameLabel.textColor = kNavigationColor;
    }
 ...
 *  展示设置密码控制器
 */
+(instancetype)showSettingLockVCInVC:(UIViewController *)vc successBlock:(void(^)(CLLockVC *lockVC,NSString *pwd))successBlock failBlock:(void (^)(CLLockVC *, NSString *))failBlock{
    
    CLLockVC *lockVC = [self lockVC:vc];
    
    lockVC.title = @"设置手势码";
    
    lockVC.navigationItem.leftBarButtonItem = lockVC.leftItem;
...
+(instancetype)showVerifyLockVCInVC:(UIViewController *)vc forgetPwdBlock:(void(^)())forgetPwdBlock successBlock:(void(^)(CLLockVC *lockVC, NSString *pwd))successBlock{
    ...
    lockVC.navigationItem.title = @"手势密码登录";
    
//    [lockVC.view addSubview:lockVC.userNameLabel];
//    lockVC.view.backgroundColor = [UIColor redColor];
...
//    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"12" message:@"123e" delegate:nil cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
//    [alertView show];
    
    UIWindow *window = [[UIApplication sharedApplication] keyWindow];
    
    [window addSubview:self.grayView];
    [self.grayView addSubview:self.remindView];
...
{
    if (!_remindView) {
        _remindView = [[RemindView alloc] initWithFrame:CGRectMake(10, 200, kScreenWidth-10*2, 150)];
        _remindView.layer.cornerRadius = 8;
        _remindView.backgroundColor = [UIColor whiteColor];
        
        ZXWeakSelf;
        [_remindView setBtnAction:^(NSNumber *number) {
            if (!number.boolValue) {
                NSLog(@"取消");
...
-(void)requestDataOfSwitch
{
    ...
    //2.比较密码是否相同
    NSString *nowPWDString = self.remindView.zsdTextField.text;
    if ([nowPWDString isEqualToString:@"123456"]) {
        //密码输入正确
        NSString *gestureString = [[NSUserDefaults standardUserDefaults] objectForKey:@"gesture"];
        NSLog(@"gestureString is %@",gestureString);
        
        
        if ([gestureString intValue] == 0) {
